/datum/software/app/communications
	...
	Display(var/mob/user as mob)
		...
		if (istype(user, /mob/living/silicon))
			return dat + DisplayAI(user)

		switch(src.state)
			if("mainmenu")
				if (M.auth.CheckAccess(access_captain))
					dat += "<BR>\[ <A HREF='?src=\ref[src];operation=announce'>Make An Announcement</A> \]"
				if(src.emagged == 0)
					dat += "<BR>\[ <A HREF='?src=\ref[src];operation=msgcentcomm'>Send an emergency message to Centcomm</A> \]"
				else
					dat += "<BR>\[ <A HREF='?src=\ref[src];operation=msgsyndicate'>Send an emergency message to \[UNKNOWN\]</A> \]"
			...
			if("crewtransfer") // this is the shiftchage screen.
	...
	proc/DisplayAI(var/mob/living/silicon/ai/user as mob)
		var/dat = ""
		switch(src.aistate)
			if(STATE_DEFAULT)
